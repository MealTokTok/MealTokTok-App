...
class SelectMenuScreen1 extends StatefulWidget {
  ...
  @override
  State<SelectMenuScreen1> createState() => _SelectMenuState();
}

class _SelectMenuState extends State<SelectMenuScreen1> {
  ...
  Future<void> fetchData() async {
    ...
    dishName = TextEditingController(text: widget.meal.mealName);
    selectedSideDish=widget.meal.dishes;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(padding:EdgeInsets.fromLTRB(0, 0, 20, 0), child: TextButton(
            onPressed: () {
              // 삭제 버튼 클릭 시 동작

              showWarningDialog(context, '\'${widget.meal.mealName}\'을\n삭제할까요?', '메뉴 구성을 삭제하면 되돌릴 수 없어요.', '돌아가기', '삭제하기', null,_Delete);
            },
            style: ButtonStyle(
              foregroundColor: MaterialStateProperty.all<Color>(SECONDARY),
              // 글씨 색상
              side: MaterialStateProperty.all<BorderSide>(
                BorderSide(color: GRAY2), // 테두리 색상
              ),
              backgroundColor: MaterialStateProperty.all<Color>(Colors.white),
              // 버튼 배경 색상
              shape: MaterialStateProperty.all<RoundedRectangleBorder>(
                RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(8.0), // 테두리 모서리 둥글기
                ),
              ),
            ),
        ...
        child: Container(
          ...
          child: ElevatedButton(
            onPressed: () async {
              ...
              await networkRequest('api/v1/meals/${widget.meal.mealId}', RequestType.PUT, {
